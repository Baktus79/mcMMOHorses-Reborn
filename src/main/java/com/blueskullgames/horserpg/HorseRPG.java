...
public class HorseRPG extends JavaPlugin {
	...
	public static void showHorse(CommandSender sender, String[] args) {
		...
		if (!DisableshowStatsInChat) {
			p.sendMessage(
					ChatColor.GREEN + "--------------------",
					ChatColor.YELLOW + "\"" + h.name + "'" + sb_STATS,
					"",
					(ChatColor.GREEN + sb_Swiftness) + ChatColor.WHITE + "  " + h.swiftness.level,
					(ChatColor.DARK_GRAY + sb_Swiftness) + ChatColor.GRAY + " " + h.swiftness.amplitude + (ChatColor.DARK_GRAY + " Time: ") + ChatColor.GRAY + " " + (h.swiftness.duration / 20) + "s",
					(ChatColor.GREEN + sb_Agility + ChatColor.WHITE + "  " + h.agility.level),
					(ChatColor.DARK_GRAY + sb_Dodge) + ChatColor.GRAY + " " + ((int) (h.agility.dodgeChance * 100)) + "% " + (ChatColor.DARK_GRAY + sb_Roll) + ChatColor.GRAY	+ " " + ((int) (h.agility.rollChance * 100)) + "%",
					(ChatColor.GREEN + sb_Vitality) + ChatColor.WHITE + "  " + h.vitality.level,
					(ChatColor.DARK_GRAY + sb_Vitality) + ChatColor.GRAY + " " + ((44 - h.vitality.healthBonus)) / 2 + (ChatColor.DARK_GRAY + " " + sb_Bonus) + ChatColor.GRAY + " " + (h.vitality.healthBonus / 2),
					(ChatColor.GREEN + sb_Wrath) + ChatColor.WHITE + "  " + h.wrath.level,
					(ChatColor.DARK_GRAY + sb_Infuriate) + ChatColor.GRAY + " " + ((int) (h.wrath.infuriateChance * 100)) + "%",
					"",
					(ChatColor.GOLD + sb_PowerLevel) + ChatColor.WHITE + "  " + h.powerLevel,
					(ChatColor.GOLD + sb_Sex) + ChatColor.WHITE + "  " + (h.isMale ? sb_Male : sb_Female) + (h.variant == Variant.DONKEY || h.variant == Variant.MULE ? sb_Gelding : ""),
					"",
					(ChatColor.GREEN + sb_BaseSpeed) + ChatColor.WHITE + "  " + h.generic_speed,
					(ChatColor.GREEN + sb_BaseJump) + ChatColor.WHITE + "  " + h.generic_jump,
					ChatColor.GREEN + "--------------------");
		}
	...
	private void initMessages() {
		...
		sb_BaseSpeed = messages.a("Scoreboard.baseSpeed", sb_BaseSpeed);
